...
export default function HRMApp() {
  ...
  const NavLink = ({ id, children }) => (
    <button onClick={() => setRoute(id)}
      className={`px-3 py-2 rounded-xl text-sm font-medium transition ${route === id ? 'bg-white text-black' : 'text-white/90 hover:bg-white/10'}`}>
      {children}
    </button>
  );
  ...
  const Hero = () => (
    <section className="relative mt-6 rounded-2xl overflow-hidden">
      <img src="/hero.jpg" alt="Remote Hands in Madrid" className="w-full h-[420px] object-cover opacity-25" />
      <div className="absolute inset-0 grid place-items-center">
        <div className="text-center px-6">
          ...
          <p className="mt-3 text-white/80 max-w-3xl mx-auto">
            {t('Expert fiber & network engineers for installs, testing and on-site interventions.',
               'Ingenieros expertos en fibra y redes para instalaciones, pruebas e intervenciones in situ.')}
          </p>
          <div className="mt-6 flex items-center justify-center gap-3">
            <a href="https://wa.me/34600906245?text=Hello%2C%20I%20need%20remote%20hands%20in%20a%20Madrid%20data%20center."
  ...
  const Services = () => (
    <section className="mt-10 grid md:grid-cols-2 gap-5">
      {[
        { title: t('Fast On-Site','Rápido en sitio'), desc: t('30–90 min to major Madrid DCs.','30–90 minutos a los principales DCs de Madrid.') },
        { title: t('Network & Fiber','Red y fibra'), desc: t('Switches, servers, cross-connects, OTDR tests.','Conmutadores, servidores, cross-connects, pruebas OTDR.') },
        { title: t('Installations','Instalaciones'), desc: t('PDUs, optics, transceivers, cabling.','PDUs, ópticas, transceptores, cableado.') },
      ].map((c) => (
  ...
  const Contact = () => (
    <section className="mt-10 rounded-2xl border border-white/10 p-6 text-center">
      ...
      <p className="text-white/80 mt-2">{t('WhatsApp 24/7 for urgent requests.','WhatsApp 24/7 para urgencias.')}</p>
      <div className="mt-4 flex items-center justify-center gap-3">
        <a className="px-5 py-3 rounded-xl bg-emerald-500 text-black font-semibold hover:bg-emerald-400"
           href="https://wa.me/34600906245?text=Hello%2C%20I%20need%20remote%20hands%20in%20a%20Madrid%20data%20center."
           target="_blank" rel="noreferrer">WhatsApp +34 600 906 245</a>
        <a className="px-5 py-3 rounded-xl border border-white/20 hover:bg-white/10"
           href="mailto:automatizacioncasola@gmail.com">Email</a>
      </div>
    </section>
  );
      ...
      <header className="sticky top-0 z-20 bg-neutral-950/70 backdrop-blur border-b border-white/10">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-2">
            ...
            <a href="https://wa.me/34600906245?text=Hello%2C%20I%20need%20remote%20hands%20in%20a%20Madrid%20data%20center."
               target="_blank" rel="noreferrer"
               className="ml-2 px-3 py-2 rounded-xl bg-emerald-500 text-black font-semibold hover:bg-emerald-400">WhatsApp</a>
            <button onClick={() => setLang(lang === 'en' ? 'es' : 'en')}
               className="ml-2 px-3 py-2 rounded-xl border border-white/20 hover:bg-white/10">{lang.toUpperCase()}</button>
          </nav>
        </div>
      </header>
